...
// JSON ファイルのパス
const beforePath = path.resolve(__dirname, './results/reservation_report.json');
const afterPath = path.resolve(__dirname, './results/reservation_report_redis.json');

// 出力 HTML のパス
const outputPath = path.resolve(__dirname, './results/reservation_comparison.html');
...
function summarize(metrics) {
    ...
    metrics.forEach(m => {
        ...
        if (m.metric === 'http_req_duration') summary[scenario].durations.push(m.data.value || 0);

        if (m.data.tags.status) {
            summary[scenario].checksTotal += 1;
            if (m.data.tags.status === '200') summary[scenario].checksPassed += 1;
        }
    });
    return summary;
}
...
<head>
<meta charset="UTF-8">
<title>Before/After 比較レポート</title>
<style>
